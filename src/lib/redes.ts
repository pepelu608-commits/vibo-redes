...
export function caption(v: Video, boteCents?: number, mesCents?: number, empiezaEn?: string | Date): string {
  ...
  if (v.pregunta) {
    ...
    const quien = bote ? (es ? " Cuantos más jugáis, más premiados." : " More players, more winners.") : "";
...
export function textoPara(red: Red, f: Fila): { texto: string; titulo: string } {
  ...
  const corto = base
    ...
    .replace(/\s*(Para los 5 mejores\.|Top 5 split it\.|Cuantos más jugáis, más premiados\.|More players, more winners\.)/g, "")
